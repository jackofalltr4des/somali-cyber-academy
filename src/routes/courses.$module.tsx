...
export const Route = createFileRoute("/courses/$module")({
  head: ({ params }) => {
    ...
    const title = mod
      ? `${mod.english} — SomTrust Cyber Academy`
      : "Module — SomTrust Cyber Academy";
      ...
      <div className="mt-4 space-y-3">
        {mod.lessonList.map((l, i) => {
          const done = (data?.progress ?? []).some(
            (p) =>
              p.module_slug === mod.slug &&
              p.lesson_slug === l.slug,
          );
